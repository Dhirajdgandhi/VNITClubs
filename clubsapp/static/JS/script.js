  ...
  $(function() {
   ...
   //  for (var i = 0; i < projectContainerList.length; i++) {
      ...
      function appendAndPrepend(info, img, close) {
        ...
        $(infoTab).prepend(img);
      }  appendAndPrepend(projectInfo,  closeButton);

      $(projectNavigation).on('click', function(event) {
        if ( event.target.tagName.toLowerCase() === 'a' ) {
          showContent(event, project, projectNavigation, projectInfo,  closeButton);
        }
      });
	...
	$('.event-desp').each(function(event) { /* select all divs with the item class */
        ...
        if ($(this).html().length > max_length) { /* check for content length */
            ...
            $(this).find('a.read_more').click(function (event) { /* find the a.read_more element within the new html and bind the following code to it */
                ...
                $(this).parents('.event-desp').find('.more_text').show();
...
$(function(){ /* to make sure the script runs after page load */
	...
	$('.photo-details p').each(function(event) { /* select all divs with the item class */
